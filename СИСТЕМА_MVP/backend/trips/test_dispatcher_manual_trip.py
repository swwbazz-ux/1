...
class HaulAssignmentReleaseTests(TestCase):
    """Назначение самосвала живёт не дольше смены экскаватора."""

    def setUp(self):
        truck_type = EquipmentType.objects.create(name='Самосвал')
        excavator_type = EquipmentType.objects.create(name='Экскаватор')
        self.truck = Equipment.objects.create(equipment_type=truck_type, garage_number='21')
        self.other_truck = Equipment.objects.create(equipment_type=truck_type, garage_number='22')
    ...
    def test_service_close_of_excavator_shift_releases_its_assignments(self):
        from trips.views import finish_service_closed_shift

        finish_service_closed_shift(
            self.shift,
            closed_by=None,
            close_kind='auto_expired',
            note='тест',
        )

        self.assignment.refresh_from_db()
        self.assertIsNotNone(self.assignment.ended_at, 'назначение снимается вместе со сменой')
        self.assertEqual(self.assignment.status, AssignmentStatus.CANCELLED)

    def test_orphan_assignments_are_released_and_live_ones_kept(self):
        from assignments.services import release_orphan_haul_assignments

        stale_excavator = Equipment.objects.create(
            equipment_type=self.excavator.equipment_type,
            garage_number='9',
        )
        stale = HaulAssignment.objects.create(
            excavator=stale_excavator,
            truck=self.other_truck,
            status=AssignmentStatus.ACCEPTED,
        )

        released = release_orphan_haul_assignments()

        stale.refresh_from_db()
        self.assignment.refresh_from_db()
        self.assertIn(stale.id, [item.id for item in released])
        self.assertIsNotNone(stale.ended_at, 'назначение без открытой смены экскаватора снимается')
        self.assertIsNone(self.assignment.ended_at, 'работающий комплекс сохраняет состав')

    def test_auto_pass_releases_orphan_assignments(self):
        from trips.views import auto_close_expired_equipment_shifts

        self.shift.opened_at = timezone.now() - timedelta(hours=30)
