...
class DispatcherSharedShiftStartTests(TestCase):
    ...
    def test_empty_dispatcher_snapshot_uses_night_production_date_after_midnight(self):
        shift = EmployeeShift.objects.create(
            employee=self.current_dispatcher,
            shift_type='night',
            opened_at=datetime(2026, 7, 24, 1, 30, tzinfo=ZoneInfo('Asia/Vladivostok')),
            plan_status='',
        )

        progress = dispatcher_empty_snapshot_progress(shift)
    ...
    def test_blocked_dispatcher_can_service_close_previous_shift_with_reason(self):
        previous_dispatcher = Employee.objects.create(
            full_name='Предыдущий диспетчер',
            phone='79000000501',
            status=Employee.Status.ACTIVE,
            is_active=True,
        )
        ...
        blocking_shift = EmployeeShift.objects.create(
            employee=previous_dispatcher,
            shift_type='day',
            opened_at=timezone.now() - timedelta(hours=13),
            opened_by=previous_dispatcher,
        )

        page = self.client.get(reverse('dispatcher_control'))

        self.assertContains(page, 'Предыдущий диспетчер')
        self.assertContains(page, 'Закрыть предыдущую смену служебно')
        response = self.client.post(
            reverse('dispatcher_service_close_shift', args=[blocking_shift.id]),
            {'reason': 'Предыдущий сотрудник не завершил смену'},
        )
        blocking_shift.refresh_from_db()

        self.assertRedirects(response, reverse('dispatcher_control'))
        self.assertIsNotNone(blocking_shift.closed_at)
        self.assertTrue(blocking_shift.is_service_closed)
        self.assertEqual(blocking_shift.closed_by, self.current_dispatcher)
        action = DispatcherActionLog.objects.get(
            action_type=DispatcherActionType.SERVICE_CLOSE_SHIFT,
            shift=blocking_shift,
        )
        self.assertEqual(action.reason, 'Предыдущий сотрудник не завершил смену')

    def test_service_close_requires_reason_and_rejects_own_shift(self):
        own_shift = EmployeeShift.objects.create(
            employee=self.current_dispatcher,
            shift_type='day',
        ...
        missing_reason_response = self.client.post(
            reverse('dispatcher_service_close_shift', args=[own_shift.id]),
            {'reason': ''},
        )
        own_shift.refresh_from_db()
        self.assertRedirects(missing_reason_response, reverse('dispatcher_control'))
        self.assertIsNone(own_shift.closed_at)

        own_shift_response = self.client.post(
            reverse('dispatcher_service_close_shift', args=[own_shift.id]),
            {'reason': 'Попытка закрыть свою смену'},
        )
        own_shift.refresh_from_db()
        self.assertRedirects(own_shift_response, reverse('dispatcher_control'))
        self.assertIsNone(own_shift.closed_at)
...
class DispatcherGarageCurrentStateTests(TestCase):
    ...
    def test_carryover_trip_is_visible_but_not_counted_in_new_shift_kpi(self):
        ...
        old_loading_shift = EmployeeShift.objects.create(
            ...
            opened_at=self.shift.opened_at - timedelta(hours=8),
            closed_at=self.shift.opened_at,
        )
        ...
        self.assertEqual(trucks_by_name['11']['equipment_state_code'], 'loaded_waiting_unload')
        self.assertEqual(trucks_by_name['11']['label'], 'На разгрузку')
        self.assertEqual(dashboard['dispatcher_kpis']['fact_tons'], '0')

        Trip.objects.filter(pk=trip.pk).update(
            status=TripStatus.COMPLETED,
            completed_at=self.shift.opened_at + timedelta(minutes=10),
        )
        completed_dashboard = self.build_dashboard()

        self.assertEqual(completed_dashboard['dispatcher_kpis']['fact_tons'], '0')
        self.assertTrue(
            Trip.objects.filter(
                pk=trip.pk,
                loading_shift=old_loading_shift,
    ...
    def test_trip_loaded_after_boundary_counts_for_actual_loading_shift(self):
        ...
        new_loading_shift = EmployeeShift.objects.create(
            ...
            shift_type='day',
            equipment=self.excavator,
            opened_at=self.shift.opened_at + timedelta(minutes=1),
        )
        trip = Trip.objects.create(
            excavator=self.excavator,
            truck=self.active_truck,
            ...
            tonnage='127.37',
            status=TripStatus.LOADED_WAITING_UNLOAD,
            loading_shift=new_loading_shift,
        )
        Trip.objects.filter(pk=trip.pk).update(
            created_at=self.shift.opened_at - timedelta(minutes=10),
        )
        ...
        self.assertEqual(dashboard['dispatcher_kpis']['fact_tons'], '127')
...
class ExcavatorWorkServerIntegrationTests(TestCase):
    ...
    def test_excavator_work_uses_project_realtime_and_reference_data(self):
        ...
        self.assertContains(response, 'excavator-mobile-shell-v133')
        self.assertEqual(response.context['app_shell_version'], 'excavator-mobile-shell-v133')
        ...
        self.assertContains(response, 'class="eo-current-app-version" aria-label="Текущая версия приложения">Версия 133</span>')
        self.assertContains(response, 'data-eo-notice-modal')
        self.assertContains(response, 'function showExcavatorNotice(message)')
        self.assertContains(response, 'data-eo-assignment-snapshot')
        self.assertContains(response, 'document.body.dataset.realtimeVisibilityOnly = "true"')
        self.assertContains(response, 'function announceExcavatorAssignmentChanges(previousSnapshot, nextSnapshot)')
        self.assertContains(response, 'Назначен самосвал ')
        self.assertContains(response, 'navigator.vibrate([180, 90, 180])')
        self.assertContains(response, 'function playExcavatorAssignmentAlert()')
        ...
        self.assertContains(response, 'class="mm-mobile-nav-icon mm-mobile-nav-icon-work"')
        self.assertContains(response, 'class="mm-mobile-nav-icon mm-mobile-nav-icon-shift"')
        self.assertContains(response, 'class="mm-mobile-nav-icon mm-mobile-nav-icon-face"')
        self.assertContains(response, 'class="mm-mobile-nav-icon mm-mobile-nav-icon-downtime"')
        ...
        self.assertNotContains(response, 'fetchServerVersion')
    ...
    def test_excavator_work_disables_apply_button_for_saved_settings(self):
        ...
        self.assertContains(response, 'Показания')
        ...
        self.assertNotContains(response, 'data-eo-shift-mileage')
        self.assertNotContains(response, '<em>км</em>')
    ...
    def test_excavator_shift_action_opens_shift_when_none_is_open(self):
        ...
        payload = json.loads(response.content.decode('utf-8'))
        self.assertTrue(payload['ok'])
        self.assertEqual(payload['action'], 'excavator_shift_opened')
    ...
    def test_excavator_shift_open_requires_fuel_and_engine_hours(self):
        ...
        fuel_response = self.client.post(
            reverse('excavator_shift_action'),
            data=json.dumps({'action': 'open', 'client_action_id': 'missing-fuel', 'fuel': '', 'engine_hours': '1200'}),
            content_type='application/json',
        )
        hours_response = self.client.post(
            reverse('excavator_shift_action'),
            data=json.dumps({'action': 'open', 'client_action_id': 'missing-hours', 'fuel': '100', 'engine_hours': ''}),
            content_type='application/json',
        )

        self.assertEqual(fuel_response.status_code, 400)
        self.assertIn('fuel', fuel_response.json()['field_errors'])
        self.assertEqual(hours_response.status_code, 400)
        self.assertIn('engine_hours', hours_response.json()['field_errors'])
        self.assertFalse(EmployeeShift.objects.filter(employee=self.operator, closed_at__isnull=True).exists())

    def test_excavator_shift_respects_model_fuel_limit(self):
        EmployeeShift.objects.filter(employee=self.operator, closed_at__isnull=True).update(closed_at=timezone.now())
        self.excavator_model.fuel_capacity_limit_l = 5000
        self.excavator_model.save(update_fields=['fuel_capacity_limit_l'])

        allowed = self.client.post(
            reverse('excavator_shift_action'),
            data=json.dumps({'action': 'open', 'client_action_id': 'fuel-5000', 'fuel': '5000', 'engine_hours': '1200'}),
            content_type='application/json',
        )
        self.assertEqual(allowed.status_code, 200)
        EmployeeShift.objects.filter(pk=allowed.json()['shift_id']).update(closed_at=timezone.now())

        rejected = self.client.post(
            reverse('excavator_shift_action'),
            data=json.dumps({'action': 'open', 'client_action_id': 'fuel-5001', 'fuel': '5000.01', 'engine_hours': '1200'}),
            content_type='application/json',
        )
        self.assertEqual(rejected.status_code, 400)
        self.assertIn('fuel', rejected.json()['field_errors'])

    def test_excavator_shift_allows_7000_and_blocks_unconfigured_model(self):
        EmployeeShift.objects.filter(employee=self.operator, closed_at__isnull=True).update(closed_at=timezone.now())
        allowed = self.client.post(
            reverse('excavator_shift_action'),
            data=json.dumps({'action': 'open', 'client_action_id': 'fuel-7000', 'fuel': '7000', 'engine_hours': '1200'}),
            content_type='application/json',
        )
        self.assertEqual(allowed.status_code, 200)
        EmployeeShift.objects.filter(pk=allowed.json()['shift_id']).update(closed_at=timezone.now())
        ...
        blocked = self.client.post(
            reverse('excavator_shift_action'),
            data=json.dumps({'action': 'open', 'client_action_id': 'fuel-no-limit', 'fuel': '100', 'engine_hours': '1200'}),
            content_type='application/json',
        )
        self.assertEqual(blocked.status_code, 409)
        self.assertEqual(blocked.json()['code'], 'fuel_limit_not_configured')

    def test_excavator_shift_open_saves_handover_correction_audit(self):
        previous = EmployeeShift.objects.get(employee=self.operator, closed_at__isnull=True)
        previous.end_fuel = '85.00'
        previous.end_engine_hours = '1208.00'
        previous.closed_at = timezone.now()
        previous.save(update_fields=['end_fuel', 'end_engine_hours', 'closed_at'])
        ...
        self.assertEqual(response.status_code, 200)
        shift = EmployeeShift.objects.get(pk=response.json()['shift_id'])
        corrections = ShiftReadingCorrection.objects.filter(new_shift=shift).order_by('metric')
        self.assertEqual(corrections.count(), 2)
        self.assertEqual({item.metric for item in corrections}, {'fuel', 'engine_hours'})
        self.assertTrue(all(item.previous_shift_id == previous.id for item in corrections))
        self.assertTrue(OperationalStateEvent.objects.filter(
            event_type='excavator_shift_readings_corrected',
            object_id=str(shift.id),
        ).exists())

    def test_excavator_shift_open_without_changed_handover_values_has_no_audit(self):
        previous = EmployeeShift.objects.get(employee=self.operator, closed_at__isnull=True)
        previous.end_fuel = '85.00'
        previous.end_engine_hours = '1208.00'
        previous.closed_at = timezone.now()
        ...
        response = self.client.post(
            reverse('excavator_shift_action'),
            data=json.dumps({'action': 'open', 'client_action_id': 'handover-same', 'fuel': '85', 'engine_hours': '1208'}),
            content_type='application/json',
        )

        self.assertEqual(response.status_code, 200)
        self.assertFalse(ShiftReadingCorrection.objects.filter(new_shift_id=response.json()['shift_id']).exists())

    def test_excavator_shift_events_use_specialized_realtime_types(self):
        EmployeeShift.objects.filter(employee=self.operator, closed_at__isnull=True).update(closed_at=timezone.now())
        opened = self.client.post(
            reverse('excavator_shift_action'),
            data=json.dumps({'action': 'open', 'client_action_id': 'realtime-open', 'fuel': '100', 'engine_hours': '1200'}),
            content_type='application/json',
        )
        ...
        self.assertTrue(OperationalStateEvent.objects.filter(event_type='excavator_shift_opened', object_id=str(opened.json()['shift_id'])).exists())
    ...
    def test_second_operator_cannot_open_shift_on_busy_excavator(self):
        ...
        response = client.post(
            reverse('excavator_shift_action'),
            data=json.dumps({'action': 'open', 'client_action_id': 'busy-excavator', 'fuel': '100', 'engine_hours': '1200'}),
            content_type='application/json',
        )
        ...
        self.assertEqual(
            response.json()['error'],
            f'Смена на технике {self.excavator} уже открыта сотрудником {self.operator.full_name}.',
        )
        self.assertFalse(EmployeeShift.objects.filter(employee=second_operator, closed_at__isnull=True).exists())

    def test_excavator_shift_open_and_close_are_idempotent(self):
        EmployeeShift.objects.filter(employee=self.operator, closed_at__isnull=True).update(closed_at=timezone.now())
        open_payload = {'action': 'open', 'client_action_id': 'same-shift-open', 'fuel': '100', 'engine_hours': '1200'}
        first_open = self.client.post(reverse('excavator_shift_action'), data=json.dumps(open_payload), content_type='application/json')
        second_open = self.client.post(reverse('excavator_shift_action'), data=json.dumps(open_payload), content_type='application/json')
    ...
    def test_excavator_shift_toggle_retry_returns_original_action(self):
        ...
        payload = {
            'action': 'toggle',
            'client_action_id': 'same-shift-toggle',
            'fuel': '100',
            'engine_hours': '1200',
        }
        ...
        self.assertTrue(first.json()['shift_open'])
        self.assertTrue(second.json()['shift_open'])
        self.assertEqual(first.json()['shift_id'], second.json()['shift_id'])
    ...
    def test_excavator_shift_close_validates_engine_hours_and_allows_refuel(self):
        shift = EmployeeShift.objects.get(employee=self.operator, closed_at__isnull=True)
        shift.start_fuel = '100.00'
        shift.start_engine_hours = '1200.00'
        shift.save(update_fields=['start_fuel', 'start_engine_hours'])
        ...
        self.assertEqual(str(shift.end_engine_hours), '1212.00')
    ...
    def test_excavator_service_worker_caches_mobile_shell(self):
        ...
        self.assertIn('excavator-mobile-shell-v133', script)
    ...
    def test_truck_loaded_reuses_same_client_action_id(self):
        ...
        self.assertEqual(second_payload['status'], TripStatus.LOADED_WAITING_UNLOAD)
        self.assertFalse(second_payload['refresh_required'])
        self.assertEqual(Trip.objects.count(), 1)
        self.assertEqual(TripClientAction.objects.count(), 1)

    def test_truck_loaded_late_retry_reports_actual_terminal_trip_status(self):
        first_response = self.post_truck_loaded(client_action_id='late-terminal-action')
        self.assertEqual(first_response.status_code, 200)
        trip = Trip.objects.get(id=first_response.json()['trip_id'])

        for status, expected_label in (
            (TripStatus.COMPLETED, 'Выполнен'),
            ...
            with self.subTest(status=status):
                ...
                retry_response = self.post_truck_loaded(
                    client_action_id='late-terminal-action',
                )

                self.assertEqual(retry_response.status_code, 200)
                payload = retry_response.json()
                self.assertTrue(payload['deduplicated'])
                self.assertEqual(payload['trip_id'], trip.id)
    ...
    def test_truck_loaded_cancel_returns_truck_to_assigned_state(self):
        no_access_response = Client().post(
            ...
            data=json.dumps({
                'client_action_id': 'cancel-without-access',
        ...
        self.assertEqual(no_access_response.status_code, 403)
        self.assertEqual(
            no_access_response.json()['error'],
            'Нет доступа к экрану Экскаваторщика.',
        )

        missing_action_response = self.client.post(
            reverse('excavator_truck_loaded_cancel'),
            data=json.dumps({
                'truck_id': self.truck.id,
                'trip_id': 1,
        ...
        self.assertEqual(missing_action_response.status_code, 400)
        self.assertEqual(
            missing_action_response.json()['error'],
            'Не передан client_action_id.',
        )

        invalid_parameters_response = self.client.post(
            reverse('excavator_truck_loaded_cancel'),
            data=json.dumps({
                'client_action_id': 'cancel-invalid-parameters',
                'truck_id': self.truck.id,
                'trip_id': 'not-an-integer',
                'dump_point_id': self.dump_point.id,
            }),
            content_type='application/json',
        )
        self.assertEqual(invalid_parameters_response.status_code, 400)
        self.assertEqual(
            invalid_parameters_response.json()['error'],
            'Некорректные параметры действия.',
        )

        missing_trip_response = self.client.post(
            reverse('excavator_truck_loaded_cancel'),
            data=json.dumps({
                'client_action_id': 'cancel-missing-trip',
                'truck_id': self.truck.id,
                'trip_id': 999999,
                'dump_point_id': self.dump_point.id,
            }),
        ...
        self.assertEqual(missing_trip_response.status_code, 409)
        self.assertEqual(
            missing_trip_response.json()['error'],
            'Незакрытый рейс для отмены не найден.',
        )

        load_response = self.post_truck_loaded(client_action_id='cancel-load')
        trip = Trip.objects.get(id=json.loads(load_response.content.decode('utf-8'))['trip_id'])
        other_dump_point = DumpPoint.objects.create(name='Другая точка разгрузки')

        dump_mismatch_response = self.client.post(
            reverse('excavator_truck_loaded_cancel'),
            data=json.dumps({
                'client_action_id': 'cancel-dump-mismatch',
                'truck_id': trip.truck_id,
                'trip_id': trip.id,
                'dump_point_id': other_dump_point.id,
            }),
            content_type='application/json',
        )
        self.assertEqual(dump_mismatch_response.status_code, 409)
        self.assertEqual(
            dump_mismatch_response.json()['error'],
            'Точка разгрузки в действии не совпадает с рейсом.',
        )
        trip.refresh_from_db()
        self.assertEqual(trip.status, TripStatus.LOADED_WAITING_UNLOAD)
        self.assertFalse(
