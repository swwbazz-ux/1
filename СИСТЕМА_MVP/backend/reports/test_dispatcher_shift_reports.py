...
class DispatcherShiftReportTests(TestCase):
    ...
    def test_reports_hub_and_both_forms_are_available(self):
        ...
        self.assertContains(trucks, 'data-dispatcher-shift-report-sync')
        self.assertContains(trucks, 'Обновлено при открытии')
        self.assertContains(trucks, 'timeoutMs: fragmentTimeoutMs')
        self.assertContains(trucks, 'var fragmentTimeoutMs = 3000')
        self.assertContains(trucks, 'var reconcileIntervalMs = 10000')
        self.assertContains(trucks, 'reason: "periodic_reconcile"')
        self.assertContains(trucks, 'Обновление задерживается — повторяем')
