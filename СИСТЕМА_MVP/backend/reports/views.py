...
def volume_report_view(request):
    ...
    trips = Trip.objects.filter(status=TripStatus.COMPLETED).select_related('truck', 'excavator', 'rock_type', 'dump_point').order_by('-completed_at')
...
def volume_report_export_view(request):
    ...
    sheet.title = 'Объемы'
    sheet.append(['Самосвал', 'Экскаватор', 'Порода', 'Точка разгрузки', 'Объем, м3', 'Выполнен'])
    for trip in trips:
        sheet.append([
            str(trip.truck),
            str(trip.excavator),
