...
`https://qa-excavator.driverform.ru/media/apk/excavator-qa-1.0.3.apk`

Это отдельный Android-пакет `ru.copperresources.excavator.qa`, версия
`1.0.3-qa (4)`, название `Экскаваторщик QA`. Он устанавливается рядом с
production-приложением и не заменяет его. APK предназначен только для
внутренней QA-проверки, не для загрузки в RuStore. Размер — `3 936 944` байт,
SHA-256 — `70eb60c9d1a4002d45efa2a21f371830799df7095a6c7f1c6b806cdb4399c183`.
...
В `1.0.3-qa` действие Android IME перехватывается на уровне `InputConnection`
собственного WebView, поэтому переход не зависит от необязательного DOM-события
`keydown`: Enter переводит фокус с топлива на моточасы, а из моточасов — на
текущую кнопку смены и немедленно освобождает нижнюю часть экрана.
