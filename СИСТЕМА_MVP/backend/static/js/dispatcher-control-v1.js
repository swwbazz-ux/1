...
document.addEventListener("DOMContentLoaded", function () {
    ...
    var detailService = document.querySelector("[data-gd-detail-service]");
    ...
    var miningMasterShellVersion = "mining-master-mobile-shell-v122";
    ...
    function renderDetailServiceActions(data) {
        if (!detailService) return;
        detailService.innerHTML = "";
        detailService.hidden = true;
        var service = data && data.service_actions ? data.service_actions : null;
        if (!service || Object.keys(service).length === 0) return;

        var title = document.createElement("h3");
        title.textContent = "Служебное управление";
        detailService.appendChild(title);
        if (!service.enabled) {
            var unavailable = document.createElement("p");
            unavailable.className = "gd-detail-service__message";
            unavailable.textContent = service.message || "Служебное действие сейчас недоступно.";
            detailService.appendChild(unavailable);
            detailService.hidden = false;
            return;
        }

        var state = document.createElement("p");
        state.className = "gd-detail-service__state";
        state.textContent = service.active
            ? "Открыт простой: " + (service.active_reason || "причина не указана")
            : "Активного простоя нет";
        detailService.appendChild(state);

        var form = document.createElement("form");
        form.className = "gd-detail-service__form";
        form.action = service.url || "";
        form.method = "post";

        if (!service.active) {
            var downtimeReason = document.createElement("select");
            downtimeReason.name = "reason_id";
            downtimeReason.required = true;
            downtimeReason.setAttribute("aria-label", "Причина простоя");
            appendSelectOption(downtimeReason, "", "Причина простоя");
            (service.reasons || []).forEach(function (reason) {
                appendSelectOption(downtimeReason, String(reason.id || ""), reason.label || "");
            });
            form.appendChild(downtimeReason);
        }

        var interventionReason = document.createElement("select");
        interventionReason.name = "intervention_reason_code";
        interventionReason.required = true;
        interventionReason.setAttribute("aria-label", "Причина вмешательства диспетчера");
        appendSelectOption(interventionReason, "", "Причина вмешательства");
        appendSelectOption(interventionReason, "device_failure", "Неисправность устройства");
        appendSelectOption(interventionReason, "no_connection", "Нет связи");
        appendSelectOption(interventionReason, "employee_unavailable", "Сотрудник недоступен");
        appendSelectOption(interventionReason, "operational_correction", "Оперативная корректировка");
        appendSelectOption(interventionReason, "other", "Другая причина");
        form.appendChild(interventionReason);

        var occurredAt = document.createElement("input");
        occurredAt.type = "datetime-local";
        occurredAt.name = "occurred_at";
        occurredAt.setAttribute("aria-label", service.active ? "Фактическое время окончания" : "Фактическое время начала");
        form.appendChild(occurredAt);

        var comment = document.createElement("textarea");
        comment.name = "intervention_comment";
        comment.required = true;
        comment.maxLength = 255;
        comment.rows = 2;
        comment.placeholder = "Что произошло";
        form.appendChild(comment);

        var result = document.createElement("p");
        result.className = "gd-detail-service__result";
        result.hidden = true;
        form.appendChild(result);

        var submit = document.createElement("button");
        submit.type = "submit";
        submit.className = service.active ? "is-close" : "is-start";
        submit.textContent = service.active ? "Завершить простой" : "Открыть простой";
        form.appendChild(submit);

        form.addEventListener("submit", function (event) {
            event.preventDefault();
            if (!form.reportValidity() || !form.action) return;
            var actionLabel = service.active ? "завершить" : "открыть";
            if (!window.confirm("Служебно " + actionLabel + " простой по этой технике?")) return;
            submit.disabled = true;
            result.hidden = true;
            var body = new FormData(form);
            body.set("action", service.active ? "close" : "start");
            fetch(form.action, {
                method: "POST",
                credentials: "same-origin",
                cache: "no-store",
                ...
                return response.json().catch(function () { return {}; }).then(function (payload) {
                    if (!response.ok || !payload.ok) {
                        var requestError = new Error(payload.error || "Действие не выполнено.");
                ...
                closeEquipmentCard();
                showDispatcherNotice(payload.message || "Служебное действие выполнено.");
                if (window.AppRealtime && typeof window.AppRealtime.wake === "function") {
                    window.AppRealtime.wake("dispatcher_service_downtime");
                }
            }).catch(function (error) {
                result.textContent = error && error.message ? error.message : "Действие не выполнено.";
                result.hidden = false;
                submit.disabled = false;
            });
        });

        detailService.appendChild(form);
        detailService.hidden = false;
    }
    ...
    function renderEquipmentCard(cardId, data) {
        ...
        renderDetailServiceActions(data);
