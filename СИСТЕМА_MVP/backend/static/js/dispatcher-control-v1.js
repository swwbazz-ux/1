...
document.addEventListener("DOMContentLoaded", function () {
    ...
    var COMPLEX_TILE_MAX = { w: 56, h: 28 };
    ...
    function refreshComplexTruckRack(rack) {
        ...
        var chosen = null;
        for (var w = COMPLEX_TILE_MAX.w; w >= COMPLEX_TILE_MIN.w; w -= 2) {
            var h = Math.max(
                COMPLEX_TILE_MIN.h,
                Math.round(w * COMPLEX_TILE_MAX.h / COMPLEX_TILE_MAX.w)
            );
            var fit = complexRackCapacity(rackWidth, rackHeight, w, h);
            chosen = { w: w, h: h, cols: fit.cols, rows: fit.rows, total: fit.total };
            if (fit.total >= tiles.length) break;
        }
        if (!chosen) {
            var f = complexRackCapacity(rackWidth, rackHeight, COMPLEX_TILE_MIN.w, COMPLEX_TILE_MIN.h);
            chosen = { w: COMPLEX_TILE_MIN.w, h: COMPLEX_TILE_MIN.h, cols: f.cols, rows: f.rows, total: f.total };
        }
