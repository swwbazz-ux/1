...
test('expired-session update migrates v233 shell with exact safe assets and preserves queue', async () => {
    ...
    const cacheMap = new Map([
        ...
        ['excavator-mobile-shell-v236', currentCache],
    ]);
    const listeners = {};
    ...
    assert.deepEqual([...cacheMap.keys()], ['excavator-mobile-shell-v236']);
...
test('fresh authenticated rendered shell precaches every exact dependency for offline reopen', {
    ...
    assert.ok(dependencies.some(path => path.includes('excavator-mobile-shell-v236')));
