...
test("driver v328 shell precaches the durable runtime and exact authenticated dependencies", () => {
    assert.match(template, /driver-offline-outbox-v2\.js/);
    assert.doesNotMatch(template, /createDriverUnloadOutbox/);
    assert.match(views, /DRIVER_SHELL_VERSION = 'driver-mobile-shell-v328'/);
    ...
    assert.match(roleApps, /shell_version='driver-mobile-shell-v328'/);
});
...
test("a legacy loaded shell reloads before adopting a fragment that requires newer assets", () => {
    ...
    function execute(loadedVersion) {
        ...
        let removals = 0;
        const node = {
            dataset: {driverFragmentShell: "driver-mobile-shell-v328"},
            remove() { removals += 1; },
        };
    ...
    assert.deepEqual(execute("driver-mobile-shell-v328"), {reloads: 0, removals: 1});
});
