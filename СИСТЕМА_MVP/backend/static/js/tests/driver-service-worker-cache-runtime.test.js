...
function renderedWorkerSource() {
    ...
    return match[1]
        ...
        .replaceAll("{DRIVER_SHELL_VERSION}", "driver-mobile-shell-v225")
        .replaceAll("{{", "{")
        .replaceAll("}}", "}")
        .replaceAll("\\\\b", "\\b")
        .replaceAll("\\\\s", "\\s");
...
async function createExpiredSessionRuntime({missingDependency = "", freshShell = false} = {}) {
    ...
    const exactDependencies = [
        ...
        "/static/js/driver-offline-outbox-v2.js?v=driver-mobile-shell-v225",
    ];
...
test("fresh authenticated install caches the exact rendered Driver dependency closure", async () => {
    ...
    const current = runtime.stores.get("driver-mobile-shell-v225");
