...
class UniversalStartTests(TestCase):
    ...
    def test_start_form_preserves_contract_and_uses_accessible_brand_markup(self):
        ...
        self.assertContains(response, 'start-page-v1.css?v=20260903-8')
    ...
    def test_start_support_hides_only_for_a_real_visual_keyboard(self):
        ...
        self.assertIn('scheduleFormReveal', script)
        self.assertIn('(scrollHost || window).scrollBy', script)
        self.assertIn('scheduleViewportReset', script)
        self.assertIn('scrollHost.scrollTo', script)
        self.assertIn('(prefers-reduced-motion: reduce)', script)
        self.assertNotIn('form.scrollIntoView', script)
        self.assertNotIn('--start-vv-height', script)
