...
class LiveRoleReadOnlyClientContractTests(TestCase):
    def test_live_role_change_disables_mutations_without_reload(self):
        ...
        # Код экрана водителя вынесен в static/js и разложен по темам,
        # поэтому контракт экрана — это шаблон вместе со всеми его частями.
        driver_static = Path(settings.BASE_DIR) / 'static' / 'js'
        driver_template = (
            Path(settings.BASE_DIR)
            / 'templates'
            / 'users'
            ...
            (driver_static / name).read_text(encoding='utf-8')
            for name in (
        ...
        self.assertIn('function submitDriverUnloadOnce()', driver_template)
        self.assertIn(
