...
## Передача 20.09.2026: экран водителя после разборки, push-канал и нативный виброотклик

Перед любой правкой экрана водителя, очереди отправки, свободного ковша или
мобильной оболочки прочитать
`ПРОГРЕСС_ПРОЕКТА/ПЕРЕДАЧА_CODEX_ПУШ_И_ВИБРО_2026_09_20.md`: там карта
изменений v288–v297, список того, что нельзя ломать (правило блокировки
обновления очередью, `realtimeVisibilityOnly`, самовосстановление, вибрация
только через `window.driverHaptic`, `select_for_update(of=('self',))`), как
читать боевой телефон по USB через logcat и постановка задачи «FCM push +
нативная вибрация» с критериями приёмки.
