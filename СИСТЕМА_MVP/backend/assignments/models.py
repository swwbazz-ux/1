...
class HaulAssignmentHandoff(models.Model):
    """Серверный переход назначения между двумя экскаваторами.

    Пока переход открыт, прежний и новый машинисты видят один и тот же срок из
    ``target_assignment.effective_at``. Право создать рейс принадлежит только
    новому экскаватору; успешный рейс завершает переход досрочно и атомарно.
    """
