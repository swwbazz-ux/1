...
class MiningMasterAssignmentsViewTests(TestCase):
    ...
    def test_mining_master_mobile_compacts_trucks_and_offers_direct_transfer_mode(self):
        ...
        self.assertContains(response, 'function schedulePinMobileTruckTransferSourceAtPoint(transfer, clientX, clientY)')
        ...
        self.assertContains(response, 'window.visualViewport.offsetTop + window.visualViewport.height')
        self.assertContains(response, 'home.classList.add("is-transfer-overlay")')
        self.assertContains(response, 'transferGrid.scrollTop = 0')
        self.assertContains(response, 'schedulePinMobileTruckTransferSourceAtPoint(transfer, sourceAnchorX, sourceAnchorY)')
        ...
        self.assertContains(response, 'Тяните самосвал в сторону · ↗ — выбрать экскаватор')
        self.assertContains(response, 'expected_assignment_state_id: transfer.assignmentStateId')
    ...
    def test_mining_master_mobile_shell_includes_pwa_install_metadata(self):
        ...
        self.assertNotContains(response, 'miningMasterUpdateCheckIntervalMs')
        self.assertNotContains(response, 'checkMiningMasterPwaUpdateSilently')
        ...
        self.assertContains(response, 'mining-master-mobile-shell-v159')
    ...
    def test_browser_mining_master_keeps_pwa_update_ui(self):
        ...
        self.assertContains(response, '>версия v159</span>')
    ...
    def test_mining_master_service_worker_caches_pwa_assets(self):
        ...
        self.assertEqual(response.status_code, 200)
        self.assertIn('mining-master-mobile-shell-v159', script)
        self.assertEqual(
            response['X-App-Shell-Version'],
            'mining-master-mobile-shell-v159',
        )
        self.assertIn(
