...
Backend M8 Apply имеет статус `COMPLETE`. Canonical API:

`apply_confirmed_settlement_preview(*, run_id, control_context, confirm_replace_manual=False)`.
...
- обязательную нормализованную DAY/NIGHT-смену для каждой строки состава и placement с provenance официальной расстановки либо явного решения для внешнего resident;
- append-only точечное исправление утверждённого плана: run, resident, смена, прежнее placement/unresolved, действие «переставить/исключить/вернуть», новый slot/bed, exact actor access, время, причина и supersede-chain;
- не более одного действующего точечного исправления на `(run, resident, смена)` и fail-closed проверку соответствия resident/run/slot/period;
- отдельную неизменяемую запись применения на `(run, смена)`, чтобы ночная и дневная команды были независимыми и идемпотентными;
- server-side календарный guard: ночная смена не раньше 13-го, дневная не раньше 14-го, без override;
- effective plan как исходные immutable строки `CONFIRMED` run плюс последние действующие точечные исправления.
