...
Если у внутреннего resident нет подтверждённого `EmployeeAccommodationBinding`, обычный опубликованный `CrewPlan` и созданный из него действующий `EquipmentAssignment` могут предложить первичную техническую группу и атомарное место. При будущем Apply делопроизводитель одновременно создаст binding и occupancy, а система сохранит снимок использованного плана, назначения и ревизии. Для external resident корпоративное назначение на технику или `WatchComposition` не синтезируется.
...
Canonical domain API — `settle_resident_on_bed`, `relocate_resident_to_bed`, `release_resident_from_bed`. Employee-based entrypoints сохранены только как адаптеры действующего внутреннего HTTP UI: они разрешают Employee в уже существующий internal resident и не создают wrapper скрыто. Canonical API поддерживает external resident без Employee, EmployeeAccess, Role и PIN; UI внешней карточки и ручного внешнего заселения пока отсутствует.
...
M7 не создаёт occupancy, не изменяет M4/M5/residents/rooms/beds/anchors и не содержит Apply или UI. Будущий Apply обязан использовать только актуальный `CONFIRMED` run и повторно проверить stale/fingerprint перед записью.
...
Нормативный порядок работы — `preview → подтверждение → Apply auto baseline → ручные корректировки → итог`. MANUAL-before-first-Apply отсутствует. Первый Apply создаёт AUTO baseline только целевого WatchPeriod. Повторный Apply требует явного предупреждения и может заменить auto baseline и MANUAL-корректировки этого периода, но не удаляет историю: прежние строки закрываются либо получают `superseded`, provenance различает AUTO run и MANUAL actor/reason. Другие и завершённые WatchPeriod не изменяются.
...
6. `SettlementPreviewRun`, cohort и их версии, если команда относится к будущему Apply;
