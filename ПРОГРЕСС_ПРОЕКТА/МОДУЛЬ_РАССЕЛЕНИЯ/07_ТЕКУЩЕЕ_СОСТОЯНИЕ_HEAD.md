...
Статус: актуализирован по clean HEAD `186cc31e05d4dd295036a641f68b662b2f3dded2`; migration leaves — `assignments.0007_equipment_assignment_provenance` и `settlement.0017_m9_preview_corrections`
Источники доказательств: принятые M4–M9, официальный источник смены, раздельное применение ночной и дневной смены, ручной сменный контур, плановая карта следующего заезда и нормативные решения ADR-031/ADR-032; production в этой актуализации не проверялась и не объявляется равной локальному HEAD
...
| Текущий локальный HEAD | `186cc31e05d4dd295036a641f68b662b2f3dded2` |
| Ветка | `main` |
| Рабочее дерево до документальных изменений | Чистое; staged/unstaged/untracked = 0/0/0 |
| Migration leaves | `assignments.0007_equipment_assignment_provenance`; `settlement.0017_m9_preview_corrections` |
| Версия ресурсов карты | `settlement-map-v34` |
| origin/main | Ahead 9, behind 0 |
| Публикация | Девять текущих локальных commits не отправлены и не развёрнуты; production не объявляется равной локальному HEAD |
...
Не реализованы и не помечаются `COMPLETE`: T1 загрузки и утверждения Excel-списка, T2 версий и истории единого реестра, T3 полного рабочего места Табельщика, UI создания/поиска/выбора внешнего жильца и сверка исторических записей фактического проживания без смены.
