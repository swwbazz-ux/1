...
Дата последней сверки отчётов: 19.08.2026
Статус: актуализирован по clean HEAD `bfd7504893005e95fbd9500eee66cb3435b7c246`; migration leaves — `assignments.0007_equipment_assignment_provenance`, `rotations.0004_arrival_roster_employee_pool` и `settlement.0017_m9_preview_corrections`
Источники доказательств: принятые M4–M9, T1.1–T1.3b, официальный источник смены, раздельное применение ночной и дневной смены, рабочий раздел табельщика и нормативные решения ADR-031/ADR-032; production в этой актуализации не проверялась и не объявляется равной локальному HEAD
...
| Текущий локальный HEAD | `bfd7504893005e95fbd9500eee66cb3435b7c246` |
| Ветка | `main` |
| Рабочее дерево до документальных изменений | Чистое; staged/unstaged/untracked = 0/0/0 |
| Migration leaves | `assignments.0007_equipment_assignment_provenance`; `rotations.0004_arrival_roster_employee_pool`; `settlement.0017_m9_preview_corrections` |
| Версия ресурсов карты | `settlement-map-v34` |
| origin/main | Ahead 14, behind 0 |
| Публикация | Четырнадцать текущих локальных commits не отправлены и не развёрнуты; production не объявляется равной локальному HEAD |

После прежнего checkpoint реализованы официальный источник смены (`assignments.0007`), структурированная смена участника состава (`settlement.0015`), независимое применение ночной и дневной смены, сменная идентичность фактического проживания (`settlement.0016`), M9 точечных исправлений (`settlement.0017`), режим «Следующий заезд» и серверная классификация ручного проживания. Эти изменения находятся только в локальной цепочке из девяти commits и не объявляются опубликованными или развёрнутыми в production.
...
Внешний жилец уже может быть участником состава, календарного закрепления и фактического проживания через канонические команды для `SettlementResident`. Миграция `0013_resident_occupancy_subject` переводит прежние записи внутренних сотрудников на точные карточки жильцов без создания `EmployeeAccess`; прямой и обратный переходы останавливаются при недоказуемых внешних данных. M6–M9, применение по сменам, ручной сменный контур и отдельный режим плановой карты реализованы. Не реализованы: UI создания и выбора внешней карточки, единый реестр T1–T3 и адаптер стажёра.
...
Сохранённый M7-план подтверждается без изменения фактического проживания, M8 атомарно применяет актуальный `CONFIRMED` план отдельно по ночной и дневной смене, а M9 хранит точечные исправления отдельной цепочкой и рассчитывает действующий будущий план. T1.1–T1.3b единого реестра завершены локально; следующий активный этап — T1.4 окончательного утверждения версии.
...
Текущий migration leaf — `settlement.0017_m9_preview_corrections`; он следует после `settlement.0016_shift_scoped_apply_and_occupancy`. Канонический API применения использует точный доступ, повторно сверяет план, отпечатки и строки выбранной смены и выполняет один атомарный пакет.
...
Реализованы: официальный источник смены, структурированная смена участника M5–M7, независимое применение ночной и дневной смены, две календарно защищённые команды интерфейса, неизменяемая история применения, сменная идентичность ручного фактического проживания, M9 и отдельный режим «Следующий заезд» с плановым перемещением, исключением и возвратом.

T1.1–T1.3b имеют статус `COMPLETE`: закрытая проверка Excel, ручная проверка строк, формирование версии из `Employee` и раздел «Рабочее место табельщика → Перевахта и состав заезда» реализованы локально. Основной источник состава — база ОУП, Excel — сверка; сотрудник без `SettlementResident` сохраняется точным FK, массовое подтверждение однозначных строк идемпотентно.

Не реализованы и не помечаются `COMPLETE`: T1.4 окончательного утверждения и замены версии, `SettlementSource`/`SettlementRevision`, T2 передачи заместителю начальника участка, T3 передачи делопроизводителю и создания чернового `SettlementCohort`, полный модуль табельщика, UI создания внешнего жильца, сверка исторического проживания без смены и production deployment.
