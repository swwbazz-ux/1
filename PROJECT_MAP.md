...
- Shell: `driver-mobile-shell-v184` (опубликован на production 03.09.2026).
...
- Shell: `excavator-mobile-shell-v191` (опубликован в production 03.09.2026).
...
### Единый каркас рабочих вкладок Машиниста экскаватора

- Вкладки `Смена`, `Забой` и `Простои` используют один внешний адаптивный
  каркас из заголовка, остаточной рабочей области и нижней зоны действия:
  `СИСТЕМА_MVP/backend/static/css/mobile-downtime-unified-v1.css`.
- Все три вкладки занимают одну и ту же область между фиксированными шапкой и
  нижней навигацией. Сам экран, его рабочие зоны и навигация не имеют
  горизонтального или вертикального диапазона прокрутки.
- Общий каркас не применяется к Водителю: условный класс добавляется общему
  шаблону `Смены` только для роли `excavator`.
- Ролевое содержимое остаётся самостоятельным: два показателя смены, настройки
  забоя и сетка причин простоя используют свои данные, но одинаковые рельсы,
  отступы, заголовок и высоту нижнего действия.
- Реализация опубликована commit `74f01978` как Excavator shell `v187`.
...
- Без открытой смены вся вкладка настроек недоступна: поля `Горизонт`, `Блок`,
  `Порода`, все точки разгрузки и `Применить настройки` заблокированы уже в
  серверной разметке. При обычном открытии предупреждения нет; только попытка
  коснуться заблокированного поля, точки или кнопки вызывает на 3,6 секунды
  небольшой центрированный toast без полноэкранного фона. Серверный ответ
  `409` остаётся независимой защитой от устаревшей страницы и прямого запроса.
...
- Базовая реализация опубликована commit `195bc5a9` как Driver shell `v181` и
  ...
  подписи точек — commit `0ea9b586`, Excavator shell `v179`; защита применения
  без открытой смены и чистое уведомление — commit `92e97c1f`, Excavator shell
  `v180`; полная блокировка вкладки до начала смены — commit `633c858f`,
  Excavator shell `v181`; показ предупреждения только по попытке
  взаимодействия — commit `6c632d31`, Excavator shell `v182`. Версии APK не
  менялись.

### Вкладка «Простой» Машиниста экскаватора

- Без открытой смены все причины простоя и действие завершения недоступны уже
  в серверной разметке. Касание зоны причин не отправляет запрос, а показывает
  на 3,6 секунды общий небольшой toast `Смена не активна. Сначала начните
  смену.` без модального фона.
- При открытой смене причина запускается одним обычным нажатием. У кнопок
  причин полностью удалены прежние pointer/touch-обработчики удержания и
  заполняющий фон; один `click` создаёт ровно один запрос запуска.
- Завершение активного простоя выполняется одним обычным нажатием, без
  удержания, заполняющего фона и модального подтверждения. На время запроса
  кнопка блокируется, а повторное или перекрёстное нажатие не создаёт второй
  POST.
- Endpoint сохраняет независимую проверку открытой смены и отвечает `409` на
  устаревший или прямой запрос без смены. Реализация опубликована commit
  `0f9145c5` как Excavator shell `v183`; версии APK, модели и БД не менялись.
- Верхний таймер показывает общий простой текущей смены. На каждой
  использованной кнопке причины дополнительно показывается накопленное время
  именно этой категории; у неиспользованных причин таймер отсутствует.
- Повторные интервалы одной причины суммируются. Переключение причины завершает
  прежний `DowntimeEvent` и открывает новый в одну временную точку, не
  переписывая историю; переходящий простой целиком сохраняет принадлежность
  исходной смене. Реализация опубликована commit `394cca8c` как Excavator shell
  `v185`; новые модели и миграции не создавались.
- Чистая механика кнопок опубликована commit `749cba2` как Excavator shell
  `v186`: удалён прежний локальный hold-контур причин и завершения, действие
  завершения переведено на `MobileShiftHold`, а его зона выровнена с соседним
  экраном `Смена`. Модели, миграции и серверная семантика простоев не менялись.
- В shell `v187` решение уточнено: удержание завершения полностью удалено,
  вкладка включена в единый каркас трёх рабочих экранов, а устаревшая геометрия
  `Забоя` и `Простоев` удалена из старого CSS вместо перекрытия новыми слоями.
