...
- Shell: `excavator-mobile-shell-v186` (опубликован в production 03.09.2026).
...
- Завершение активного простоя использует общий с экраном `Смена` компонент
  удержания и его адаптивную геометрию. Короткое касание показывает только
  небольшой toast `Удерживайте кнопку`; полноэкранное окно `Подтвердите
  действие` на вкладке не вызывается.
