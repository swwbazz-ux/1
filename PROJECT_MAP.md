...
- На production 01.09.2026 опубликованы подписанные профили: Водитель
  `0.1.8 (10)` (`driver-10.apk`) и Машинист экскаватора `0.1.12 (15)`
  (`excavator-15.apk`). Вместе с APK опубликованы серверный native handoff и
  host-specific `assetlinks.json`; исходный commit — `eb7490fb`.
- Передача известного номера после прямой установки APK: `/start/` выдаёт
  короткоживущий одноразовый непрозрачный билет, а приложение принимает его
  только через проверенный HTTPS App Link `/native-handoff/open/` своего
  ролевого домена. Номер телефона, PIN и сессия в URL не передаются.
- Серверные точки механизма: `users/native_handoff.py`,
  `/.well-known/assetlinks.json`, `/native-handoff/open/`,
  `/native-handoff/redeem/`; экран после установки —
  `templates/users/native_handoff.html`.
- После скачивания пользователь завершает системную установку, возвращается
  на `/start/` и нажимает `Открыть приложение`: Package Installer сам не
  переносит контекст скачивания в новое приложение.
