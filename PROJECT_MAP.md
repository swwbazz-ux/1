...
- Водитель и Машинист экскаватора используют одинаковые компонентные классы
  для заголовка, назначения, показаний, итогов, действий и иконки «Смена».
- Компоновка строится по естественной высоте содержимого через
  `grid-auto-rows: max-content`, адаптивные `clamp/minmax` и один внутренний
  вертикальный скроллер; правил под конкретные размеры снимков нет.
