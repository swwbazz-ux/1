...
- Shell: `excavator-mobile-shell-v178` (опубликован в production 02.09.2026).
...
- Поля `Горизонт`, `Блок` и `Порода` используют тот же визуальный примитив,
  размеры, типографику и состояния, что поля показаний смены; серверные имена,
  справочники и payload сохранены.
...
- Реализация опубликована commit `195bc5a9` как Driver shell `v181` и
  Excavator shell `v178`. Версии APK не менялись.
