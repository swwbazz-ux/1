...
- Shell: `excavator-mobile-shell-v181` (опубликован в production 02.09.2026).
...
- Без открытой смены вся вкладка настроек недоступна: поля `Горизонт`, `Блок`,
  ...
  серверной разметке. В центре рабочей области показывается небольшое
  предупреждение без полноэкранного фона; серверный ответ `409` остаётся
  независимой защитой от устаревшей страницы и прямого запроса.
...
- Базовая реализация опубликована commit `195bc5a9` как Driver shell `v181` и
  ...
  Excavator shell `v181`. Версии APK не менялись.
