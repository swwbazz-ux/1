...
- Shell: `excavator-mobile-shell-v185` (опубликован в production 02.09.2026).
...
- При открытой смене причина запускается одним обычным нажатием. Отдельное окно
  `Подтвердите действие` удалено только из этого обратимого оперативного
  действия; завершение активного простоя по-прежнему требует удержания.
